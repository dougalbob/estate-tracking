...
| | |
|---|---|
| Live release | **v0.2.8** — an Event log tab listing every call, email, letter, web form and note most recent first, and one optional project on an interaction as well as a task — built by GitHub Actions and published to GHCR as `v0.2.8`, `latest`, and a git-SHA tag. Until Unraid pulls the new image the server is still running **v0.2.7** |
| Earlier releases | v0.2.0 – v0.2.7, all published 16–17 September 2026 |
...
| Verified by hand, not in a browser | The v0.2.5, v0.2.6, v0.2.7 and v0.2.8 changes were checked by rendering the real components to markup and by HTTP against the development server; the sandbox has no browser, so no click-through test has ever run in one |
| Automated gates | 87 unit/integration tests, TypeScript, Prettier and the production build all pass on the released commit |
...
The first four of those — main contact, phone numbers, email, account/reference — appear in the small popup that a task, document or event row's contact name opens, with the same labels and in the same order, so a number can be dialled or copied without leaving the list ([Tasks, dates, and projects](#tasks-dates-and-projects)). They are the contact screen's own field list too, with tap-to-dial numbers and copy buttons, because the popup and the contact screen render one shared component and cannot drift apart.
...
git tag -f -a v0.2.8 -m "v0.2.8 – <what changed>" origin/main
