...
|                                    |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Live release | **v0.2.21** — fix backup filename client override – save-as now uses version and London HHmm from server header, with a matching client fallback. Expected filename: `estate-backup-v0.2.21-YYYY-MM-DD-HHmm.estate-backup`. Sidebar version and backup metadata remain in step. Unraid was last confirmed on v0.2.20; Force Update and the Linux Mint save-dialog / PC and mobile sidebar checks remain to be done after publication |
| Earlier releases                   | v0.2.0 – v0.2.20, all published 16–20 September 2026                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| Verified by hand, not in a browser | The v0.2.5, v0.2.6, v0.2.7, v0.2.8, v0.2.9, v0.2.10, v0.2.11, v0.2.12, v0.2.13, v0.2.14, v0.2.15, v0.2.16, v0.2.17, v0.2.18, v0.2.19 and v0.2.20 changes were checked by rendering the real components to markup and by HTTP against the development server; the sandbox has no browser, so no click-through test has ever run in one                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| Automated gates                    | 189 unit/integration tests, TypeScript, Prettier and the production build all pass on the released commit, and `npm audit --omit=dev` reports 0 findings                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
- Every dialog returns focus to the button that opened it when it closes, so the keyboard never lands at the top of the page.
...
- The **Family & friends** tab sits between Estate finances and Checklists. Desktop view is a dense, keyboard-driven spreadsheet grid for rapid data entry without dialogs; mobile view is a focused call sheet with tap-to-dial links and a Contacted tick for evening phone calls.
...
Every dropdown that offers a contact lists them **A to Z**: the task and note form's Organisation list, the document edit form's Contact picker, the upload dialog, the document link picker, the financial record form, and the Event log's organisation filter. Each keeps its own fixed options where they were — “No organisation” or “Pick contact…” first, and “+ New contact…” last in the task and note form — and the order comes from one shared comparator, the same one the Contacts tab's A>Z option uses, so the list and the pickers cannot disagree. The tab's default **Added order** is untouched, because each picker sorts its own copy of what the page already holds rather than asking the database to reorder it.
...
**An interaction reads the same everywhere, and it leads with what happened.** A contact's Interaction history, Unfiled notes, the Event log, a project's journal and the overview's activity list all show one shared card: the kind as an icon *and* a word, then the **detail in full** as the first thing on the card and as the tap target that opens the record, then one line carrying the contact (tappable, opening the popup), the project, the date and time, and who recorded it. Any linked documents appear under it as rows with **View**, opening the in-app viewer. Edit, History and Bin stay in the row's actions.

**The title is not shown on a card.** A title is written when the work is set up — “Call the Co-op about the floral arrangement” — so by the time the call has happened and been written up, the title describes something that is already done and can read, at a glance, as though it has not been. The detail is required on every interaction, so the card is never empty without it; the title stays a quiet handle for the places that need a short name: the editor, the upload and link pickers, search, the revision history and the recoverable bin. A quick note may have no title at all, which changes nothing on the card.

### Linking work to a contact from either direction

A contact screen shows its linked tasks and documents together. **Linked tasks opens with done tasks hidden**, so what is still to do for that contact is what you see first. A **Show Done (n)** button sits beside **Link existing task** — *n* is how many of that contact's tasks are done and out of sight — and pressing it brings them back and changes the label to **Hide Done**; it is the same Eye/EyeOff, `aria-pressed` control as the contacts list's **Hide resolved** and a project's Hide Done. Done only: a **Cancelled** task stays in the list, because "we decided not to" is still worth seeing against the contact. The button only appears once the contact has a task (a contact whose tasks are all still open reads **Show Done (0)**); a contact with no tasks keeps **"No linked tasks yet."**, and a contact whose every task is done says so — *"Every task linked to this contact is done, so the list is empty while done tasks are hidden."* — rather than showing an empty panel. The choice is for the contact in front of you: opening another contact starts with its done tasks hidden again, and nothing is remembered across a reload. **Link existing task** attaches a task that already exists, instead of adding another one:
...
- **Create interaction**, in a task's edit dialog, saves the task and then opens the ordinary interaction editor for the same contact with the title, project, type and outcome already filled in: a Call task becomes a call, an Email task an email, and the rest become a note. Nothing reaches the event log until that interaction is saved, and every part of it can be changed first. The only thing the button waits for is an outcome, because an interaction cannot be saved without one. If the task's contact is being made in the same breath — **+ New contact…** — one press does the lot: the task is saved, the contact is created and linked in that same save, and the interaction opens already pointed at the contact that now exists. The interaction stands on its own afterwards, exactly as if it had been typed by hand — with two additions. **Documents follow the work:** when that interaction is saved, every document linked to the task is linked to the new interaction as well, in the same save, as ordinary links — they show wherever a document's links show and can be removed from the interaction by hand without touching the task's own. And the interaction quietly remembers which task it came from, so the **task row gains an “Outcome logged: 19 Sept 2026” hop** that opens the interaction — named by the day it happened, never by its title. This is inheritance, not a rule: nothing about the task changes, an interaction written by hand has no source task and is left alone, and a task marked done without an interaction is still a deliberate outcome that nothing questions. Binning the task later leaves the interaction and its documents exactly as they were.
- A form opened from another form is a **fresh form**. The button keeps itself current as you type, so the order does not matter: write the outcome before choosing the contact or choose the contact first, and clearing the outcome puts the button back to waiting. Choosing **+ New contact…** and then creating the interaction used to leave the new note's dropdown offering to create the contact again, and re-using the contact, follow-ups or version of the task that had just been saved; the editor is now keyed so a new record always starts clean, and a contact saved a moment ago is named in the dropdown from the moment the form opens rather than after the next refresh lands.
- No automatic calculation of legal deadlines.
- Reminders are in-app only; no email or browser/push notifications.
- **Projects collapse and expand** from the title row, so a long list can be scanned quickly. Collapsed is per-panel, starts expanded, and hides everything below the heading — organisation pills, documents, checklist and journal — leaving just the title row. The control is a small button with a chevron before the project name, with an accessible name and `aria-expanded`. The toolbar also carries **Collapse all** and **Expand all** beside **Add project**, so the whole list can be folded or opened in one press: each is greyed out when there is nothing left for it to do — nothing expanded to collapse, nothing collapsed to expand — and neither changes what the per-panel buttons do.
- **A project panel shows its journal**, not a list of tasks: its interactions, its documents and its tasks in one list, most recent first, so the panel tells the story of the project rather than only what is left to do. Each entry opens what it names — an interaction's detail opens the record, a document's name opens it, a task's title opens the task — and each entry sits where it happened: an interaction on the day it took place, a task on the day it was set (**not** its due date, which is shown as context beside its status in words and its assignee), and a document on the date printed on it, or on the day it was added when it has none ([Documents](#documents)). The documents strip, the **Attach doc** and **Link existing** buttons and the project's checklist all stay exactly where they were.
- Nothing is generated for the journal: every interaction in it was written by one of you. A task with no interaction is a deliberate outcome, not a gap — “choose the music” was decided and needs no note — so marking a task done is never blocked or questioned, and a completed task simply appears twice: once as the task, dated when it was set, and once as the interaction, dated when the work was done.
- **Cancelled tasks stay in the journal** with the word Cancelled, because “we decided not to” is part of the story too. Hide Done hides done tasks only.
- Each project has a **Hide Done** filter that hides tasks whose status is Done in that project, and **hiding is the default**: every panel opens with its done tasks out of sight, so the journal reads as what has happened and what is still to do, without the debris of finished tasks. The button therefore reads **Show Done (2)** first — the count is how many done tasks that project has hidden — and pressing it shows them and flips the label to **Hide Done**; it is the same Eye/EyeOff, variant and `aria-pressed` pattern as the contacts **Hide resolved** filter, per panel, and not remembered across a reload. The count under the title follows the filter: while done tasks are hidden it counts what is left and says “5 outstanding tasks”, so a smaller number reads as filtered rather than as tasks gone missing, and with them showing it reads the true total (“7 tasks”). Both counts pluralise properly (“1 task”, “1 outstanding task”), a project with no tasks reads “0 tasks” either way, and the organisations and docs counts are never affected, because Hide Done is a task filter only. Because hiding is now the default, a project whose every task is done opens on the explanation rather than on a bare list: “Every task in this project is done, so the list is empty while done tasks are hidden.”; a project with nothing in it at all reads “Nothing has happened in this project yet.”
- The **All tasks** list's **Assigned to** filter reads **All assignees** for the no-filter choice, offers each user, then **Everyone (shared)** for work that belongs to both of you together and **Unassigned** — so the one word "Everyone" means the same thing everywhere in the app, and the shared tasks can be listed on their own, as they can on the calendar.
...
- Store files locally, including PDFs, screenshots, and scanned documents. One shared rule decides which files the app stores — PDF, JPEG, PNG, WebP, TIFF, HEIC/HEIF and text, by MIME type or by file-name extension — and the share target, the upload button, the upload route and every file picker's offered files (including the receipt picker on a financial record) all read that one list, so they cannot disagree. An SVG image is refused however it arrives (an SVG can carry scripts, and documents are served from the app's own origin), and any SVG stored before this rule downloads as a file rather than opening inline.
- Upload independently or while recording an interaction.
- Give documents friendly names and categories.
- A document can carry **its own date** — the date printed on the paper: a registration, an issue date, a statement period-end, the date a deed was executed. It is optional and it is always visible where the document is: on its row, in the Upload dialog and in the document's edit form. Leave it blank and the document sits where it was added; date a house deed 1987 and it sinks to the end of a project's journal as ancient history, whatever day it was filed. The date belongs to the document rather than to any one link, so a deed linked to two projects has one date. Nothing is filled in for documents stored before this existed, and their behaviour is unchanged: blank means “the date it was added”.
...
Every refusal an upload can produce comes from one shared list of plain sentences — no file chosen, too large, unsupported type, a disk that is full, a file the server could not save — so a failure says what happened and what to try next. The technical detail for any of the above is in the browser's console and the container's `[upload]`/`[upload:api]` log lines described in the upload section.
...
- **The balance is shown while the amount is typed.** Recording a payment or a reimbursement says what the figure being entered does to the balance — “£300.00 of the £500.00 still owed – leaves £200.00.” — and an amount that is too large switches **Record** off with a plain sentence rather than being refused after the fact. Sale proceeds are a fact and have no ceiling, so nothing is limited there, and a distribution or income record has no movement form at all. The figure is worked out from the records on screen, so the server checks the same rule again on save and remains the last word.
...
A dedicated guest list and call sheet for funeral and wake attendance tracking, splitting evening phone calls, and gathering headcounts for the celebrant, order of service printing, chapel seating, and wake caterers.

### The two screens

- **Desktop spreadsheet grid:** A dense, keyboard-driven data grid designed for transcribing an existing address book, Christmas card list, or pile of envelopes without modal dialogs.
  - One row per household, plus a persistent blank trailing row for adding new households directly by typing.
  - Keyboard navigation across cells with Tab, Shift+Tab, Enter, Shift+Enter, and arrow keys.
  - Per-cell auto-save on blur or row change, with subtle status indicators (saving dot, brief saved tick, or error message) rather than disruptive toast banners.
  - Inline dropdowns for RSVP states ("Not asked", "Awaiting reply", "Coming", "Not coming", "Not sure") and Ringing assignments ("Alex", "Jamie", "—").
  - Summary strip above the grid showing live headcounts: Total households, Total party size, Funeral attendees, Wake attendees, and Dietary requirements flagged.
  - Filter bar to narrow by caller ("All", "Alex", "Jamie", "Mine", "Unassigned"), attendance ("All", "Attending funeral", "Attending wake", "Awaiting replies", "Uncontacted"), and text search.
- **Mobile call sheet (≤700px):** A focused, touch-friendly screen tailored for dividing up evening phone calls from the sofa.
  - Displays household name, party size, and relationship.
  - Prominent one-tap `tel:` button to dial directly from a mobile handset.
  - Simple "Contacted" tick box to mark progress immediately after a call without opening an edit form.
  - Tapping the household row opens a clean, read-only detail card displaying phone numbers, address, email, notes, dietary requirements, and current RSVP statuses.

### Deliberate exception to the house style

The desktop view of Family & friends is an intentional exception to the application's prevailing design pattern of warm card panels, section prose, and modal edit dialogs. Entering dozens of relatives and friends one dialog at a time is slow and exhausting. The dense spreadsheet grid prioritises data-entry velocity, compact row heights, and inline editing while preserving full keyboard accessibility.

### Complete data isolation

Households and gatherings live in their own dedicated tables (`households` and `gatherings`, migration `0011_guest_list`). They are completely isolated from professional organisations, contacts, interactions, tasks, and journals:
- Households never appear in professional contact pickers, upload link targets, or task assignment dropdowns.
- Changes to households are audited in revision history and support soft-deletion to the recoverable bin, restore, and permanent deletion, but never generate noise in the main Event log.

### Demo seed

To populate realistic family and friends data in the local development database:

```bash
DATABASE_PATH=./data/demo.sqlite npx tsx scripts/seed-demo-guests.ts
```

The script populates sample households across extended family, old neighbours, and friends with varied party sizes, dietary requirements, and RSVP statuses, as well as the two standard gatherings (Funeral service and Wake). It refuses to run against a database path that does not contain `demo`.
...
- Deleting an organisation must not cascade-delete its interactions, documents, or finances. Permanently deleting an organisation or project unlinks its financial records — the record, its figures and its history stay; only the link goes, exactly as for tasks and notes.
...
- **Create encrypted backup:** enter a recovery password of at least 12 characters. The server uses SQLite's online backup API while holding a write lock, copies the database snapshot and every regular file below `/data/documents`, then encrypts a versioned bundle with scrypt-derived AES-256-GCM. The bundle's `metadata.json` records `appVersion` (e.g. `0.2.20`) from `src/lib/version.ts`, and the browser receives an `.estate-backup` download named `estate-backup-v{version}-YYYY-MM-DD-HHmm.estate-backup` using 24h London time (`Europe/London`) so same-day backups do not overwrite and the producing release is visible for rollback. Arena may still block the final save action, but the HTTP response and encrypted bytes can be tested independently.
...
DATABASE_PATH=./data/demo.sqlite npx tsx scripts/seed-demo-guests.ts     # optional
DEV_AUTH_ENABLED=true NEXT_TELEMETRY_DISABLED=1 npm run dev
```

The demo seeds are optional and idempotent. `seed-demo-contacts.ts` writes six fictional contacts, each with a task to hang it on: one with every field filled in — a fictional map link and two phone numbers typed in different formats — one with an email and reference missing, and one with nothing added at all, so "Not added" and the copy-only behaviour can be seen; two resolved contacts, so the Hide resolved filter counts more than one and the finished task beside them shows that a done task awaits nothing; and one whose meeting task belongs to Everyone, so the shared assignment can be seen in the lists. `seed-demo-documents.ts` (v0.2.7) writes four fictional documents with small generated images — one linked to a single contact, one to two contacts, one only through a note, and since v0.2.17 a house deed dated 1987 and linked to the Probate project, so a document with a date of its own and its place at the bottom of that project's journal can both be seen; it needs the contacts seed first. `seed-demo-events.ts` (v0.2.8) writes nine fictional interactions across all five kinds, both recorders, and contacts and projects with and without — one sharing its title with a seeded document — so the Event log's search and filters can be exercised. `seed-demo-calendar.ts` (v0.2.12) writes thirteen fictional tasks with due dates counted from the day it is run, so the demo always has something in the current month: two tasks on one day, one already overdue, some further ahead and one next month, one each of In Progress and Cancelled (which the calendar never shows, so their absence can be checked) and one with no due date at all, which stays in All tasks. Because those tasks join the All tasks list too, the “(3)” in the v0.2.11 Hide Done/Scheduled example is what the contacts seed gives on its own. `seed-demo-guests.ts` (v0.2.19) writes nine fictional households and two gatherings (Funeral service and Wake) across varying party sizes, dietary requirements, ringing assignments, and RSVP statuses, so the dense PC spreadsheet grid, summary strip, filter bar, and mobile call sheet can be exercised. Like the other seeds, each refuses to run against a database path that does not contain `demo`.
