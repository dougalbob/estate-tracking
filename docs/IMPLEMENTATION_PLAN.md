...
| Item | Position today |
|---|---|
| Live release | **v0.2.8** (an Event log tab over every interaction, most recent first, and one optional project on an interaction), built by GitHub Actions and published to GHCR; v0.2.0–v0.2.7 preceded it on 16–17 September 2026, and v0.2.7 is what the server runs until the image is pulled |
...
| Automated gates | **87 unit/integration tests**, `tsc --noEmit`, `prettier --check` and `next build` all pass on the released commit |
...
| Version numbering | The next release after v0.2.8 is v0.2.9. Tags are the version source of truth; `package.json` is kept in step for tidiness |
...
- **There is no browser**, so UI changes are verified by rendering the real components to static markup with `tsx` against the demo database, plus `curl` against the development server. The harness lives outside the repository and is deleted afterwards: stub `@/app/actions` and `next/navigation`, point a temporary tsconfig at the stubs, and assert on presence *and* absence (see the v0.2.5 and v0.2.6 checkpoints).
...
| 2. Shared records | Implemented – core workflow, projects, and recoverable bin | v0.2.8 added the Event log tab over every interaction and one optional project on an interaction; browser e2e still needs a machine with a browser, so bin/project flows are covered by unit tests and manual use |
...
- **Document rows (option A1).** Each *directly linked* contact name becomes its own tappable item; note, task and project names on that line stay plain text. A contact reached only through a linked note or task is not shown — the row shows direct links, exactly as before, and the contact screen keeps the wider aggregation.
