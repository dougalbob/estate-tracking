...
| Item | Position today |
|---|---|
| Live release | **v0.2.6** (the contact popup on a task row), built by GitHub Actions and published to GHCR; v0.2.0–v0.2.5 preceded it on 16–17 September 2026, and v0.2.5 is what the server runs until the image is pulled |
...
| Automated gates | **70 unit/integration tests**, `tsc --noEmit`, `prettier --check` and `next build` all pass on the released commit |
...
| Version numbering | The next release after v0.2.6 is v0.2.7. Tags are the version source of truth; `package.json` is kept in step for tidiness |
...
Not covered here:
- Making contact names tappable on finance rows, note rows, document rows or reimbursement lines, and any change to the contact screen itself — all agreed out of scope for this pass, not deferred bugs.
