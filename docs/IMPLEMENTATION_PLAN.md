...
| Item | Position today |
|---|---|
| Live release | **v0.2.7** (contact names tappable on document rows, and dial/copy on the contact screen), built by GitHub Actions and published to GHCR; v0.2.0–v0.2.6 preceded it on 16–17 September 2026, and v0.2.6 is what the server runs until the image is pulled |
...
| Automated gates | **83 unit/integration tests**, `tsc --noEmit`, `prettier --check` and `next build` all pass on the released commit |
...
| Version numbering | The next release after v0.2.7 is v0.2.8. Tags are the version source of truth; `package.json` is kept in step for tidiness |
