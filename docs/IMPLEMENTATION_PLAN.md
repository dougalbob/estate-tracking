...
| Version numbering | The next release after v0.2.9 is v0.2.10. Tags are the version source of truth; `package.json` is kept in step for tidiness |
...
| 2. Shared records | Implemented – core workflow, projects, and recoverable bin | v0.2.8 added the Event log tab over every interaction and one optional project on an interaction; v0.2.9 added a hand-typed map link on each contact, a Notes label on the contact screen and the Event log's kind icons on interaction cards; browser e2e still needs a machine with a browser, so bin/project flows are covered by unit tests and manual use |
...
- `src/components/record-summary.tsx`: readable history and comparison fields, aware of deleted records, project links, and documents. v0.2.9 captions the new column “Map link” so a revision reads plainly.
...
Not covered here:
- **Sharing a document** (the device's share sheet, so a file can reach Gmail or WhatsApp) is deliberately left to v0.2.10: it can only be proven on a real device, and must not hold up a release that is otherwise ready.
- The published image is built by GitHub Actions from the tag, not in the sandbox.
- The accessibility and security review is still the outstanding item from the release gate; this change follows the app's existing focus, contrast-with-text and tap-target rules but does not substitute for that review.
